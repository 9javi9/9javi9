# 👋 Hi, I'm Javier Jaimes

🎓 Civil Engineer | 🤖 AI & Structural Design Automation  
📍 Lima, Peru  

---

## 🚀 About Me

Civil Engineer and AI researcher working at the intersection of:

- Spatial Data Science  
- Remote Sensing (SAR & Optical)  
- Structural Engineering & Automation  
- Seismic Risk & Urban Resilience  

I develop **data-driven and automated engineering solutions**, integrating:

> Machine Learning + Geospatial Data + Structural Modeling

---

## ⚙️ Focus

- Structural Design Automation (ETABS, SAP2000, SAFE APIs)  
- AI-assisted Engineering Workflows  
- Parametric & Performance-Based Design  
- Infrastructure Optimization  
- Risk-informed Urban Systems  

---

## 🧠 Research

📄 **Semi-Supervised Urban Segmentation (NHESS - Preprint)**  
https://egusphere.copernicus.org/preprints/2026/egusphere-2025-5987/

📄 **SAR-based Informal Growth Monitoring (WCEE 2024)**  
https://drive.google.com/file/d/17Ba3txOoCQ25NWlVxCdy_FLW2ArHn39j/view

📄 **Vulnerability of recent Informal Urban Growth (PDISAS)**  
https://doi.org/10.1016/j.pdisas.2024.100345

---

## 🏗️ Engineering & Tech

- Python (PyTorch, Scikit-learn, GDAL)  
- QGIS, SNAP, Remote Sensing workflows  
- OpenSeesPy, OpenQuake  
- Revit + Dynamo (automation)  

---

## 🎓 Education

- Civil Engineering — PUCP (Top GPA)  
- M.Sc. Artificial Intelligence — UNI (ongoing)  
- M.Sc. Natural Risk Management — PUCP (Top GPA)  

---


## 🧑‍🏫 Teaching

Teaching Assistant at PUCP:

- Physics I–IV  
- Multivariable Calculus  
- Dynamics  
- Strength of Materials  
- Structural Analysis

---

## 💼 Experience Highlights

### 🏢 BIM & Structural Design
- Automated structural workflows using Python, C#, and Dynamo  
- Developed API-based engineering tools  
- Parametric infrastructure design  

### 🧪 Research Projects (CONCYTEC – PROCIENCIA)
- GeoAI applications for urban risk  
- Satellite data processing pipelines  
- ML models for spatial classification  

---

## 🎯 Career Interests

I am seeking opportunities in:

- Structural Engineering (Design & Analysis)  
- Digital Engineering & BIM Automation  
- AI Applications in Civil Engineering  
- Infrastructure Innovation & Smart Construction  
- Research positions in GeoAI, Risk, and Urban Systems  

Particularly interested in working within **multidisciplinary and international teams**.

---

## 🌐 Portfolio

👉 https://9javi9.github.io/Portfolio_Civil_Engineer/

---

## 📫 Contact

- LinkedIn: https://linkedin.com/in/javier-alonso-jaimes-cucho-50a59a1b4  
- Email: javier.jaimes@pucp.edu.pe  

---

## 🚀 Vision

> Bridging AI and Civil Engineering to build smarter and more resilient infrastructure.
